...
from package.funcmovefile import movefile
...
while i < nombredefichiers:
	...
	print ("Dernière modification le: %s" % time.ctime(os.path.getmtime(mypath+fichiersdansdossier[i])))
	print ("Créé le: %s" % time.ctime(os.path.getctime(mypath+fichiersdansdossier[i])) )
